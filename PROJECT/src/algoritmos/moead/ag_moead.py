...
def ejecutar_algoritmo_genetico(comida_bd, objetivo_calorico, edad, grupos_alergia, grupos_gusta, grupos_no_gusta, seed,
                                n_gen=100, prob_cruce=0.9, prob_mut=1/77,
                                n_neighbors=30, prob_neighbor_mating=0.9, n_partitions=12, met_ref="incremental"):
    ...
    ref_dirs = get_reference_directions(met_ref, 3, n_partitions=n_partitions)

    algoritmo = MOEAD(  # Algoritmo MOEA\D
        ref_dirs=ref_dirs,  # Direcciones de referencia
        n_neighbors=n_neighbors, # Numero de vecinos
        prob_neighbor_mating=prob_neighbor_mating,   # Probabilidad de compartir informacion con vecino
        sampling=CustomIntegerRandomSampling(problema), # Inicializacion personalizada
        crossover=TwoPointCrossover(prob=prob_cruce),  # Cruce
